...
folder are mapped to the application's root namespace, 'TracekitAdmin'. This is set in
`"app.json"`:

    "sass": {
        "namespace": "TracekitAdmin"
    }
...
included in the application's JavaScript code build. By default, files in this 
folder are mapped to the application's root namespace, 'TracekitAdmin' in the same way
as `"TracekitAdmin/sass/src"`.
